...
- tạo lịch khám (xử lí đồng bộ khi có cả hai ngừoi cùng thực hiện đặt lịch) **(ok)**
- tạo thanh toán (diễn ra cùng với việc update trạng thái thành đã thanh toán) **(ok)**
- tạo đơn thuốc (sẽ diễn ra cùng lúc với tạo chi tiết đơn thuốc) => nếu thuốc hết hoặc gặp vấn đề giữa quá trình đó thì rollback **(ok)**
- tạo phiếu khám (sau khi đã khám xong thì phải diễn ra đồng thời update cái trạng thái thành đã khám) **(ok)** 
- tạo đơn thuốc (thêm thuốc vô đơn phải diễn ra cùng lúc với việc trừ thuốc trong kho) **(ok)**
