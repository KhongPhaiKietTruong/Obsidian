dịch: điểm tiêu chuẩn
tên khác: Z statistic

là một đại lượng dùng để chỉ ra giá trị đang xét cách [[Expected value (Giá Trị Kỳ Vọng)| giá trị trung bình]] bao nhiêu lần [[Standard deviation (Độ Lệch Chuẩn)| độ lệch chuẩn]]
công thức tính:
$$z = \frac{x - \mu}{\sigma}$$

Trong đó:
- $x$: Giá trị dữ liệu cần kiểm tra.
- $\mu$ (Mu): Giá trị trung bình của tập dữ liệu.
- $\sigma$ (Sigma): Độ lệch chuẩn của tập dữ liệu.

- Khoảng $68\%$ dữ liệu sẽ có z-score nằm trong khoảng từ $-1$ đến $+1$.
- Khoảng $95\%$ dữ liệu sẽ có z-score nằm trong khoảng từ $-2$ đến $+2$.
- Khoảng $99.7\%$ dữ liệu sẽ có z-score nằm trong khoảng từ $-3$ đến $+3$.
- _Nếu z-score lớn hơn $+3$ hoặc nhỏ hơn $-3$, giá trị đó được coi là ngoại lai (outlier) cực kỳ hiếm gặp._

![[Pasted image 20260516081740.png]]